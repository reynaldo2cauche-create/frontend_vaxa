...
// FIX: Usar texto estático de la BD en lugar de _cuerpo
// ============================================
...
export async function POST(
  ...
  try {
    ...
    const textoEstatico = body.textoEstatico || null; // 🆕 Nuevo parámetro para texto estático
    ...
    // 3. 🆕 OBTENER TEXTO ESTÁTICO - PRIORIDAD: body > BD > valor por defecto
    console.log(`📋 === DEBUG OBTENIENDO TEXTO ESTÁTICO ===`);
    ...
    const textoEstaticoBD = certificado.datos.find(d => d.campo === '_texto_estatico'); // 🆕 Buscar texto estático en BD

    // 🆕 PRIORIDAD PARA TEXTO: body > BD > valor por defecto
    if (textoEstatico) {
      // Usar texto estático del body si se proporciona
      textoCompletado = textoEstatico;
      console.log(`✅ Usando texto estático del body: "${textoCompletado.substring(0, 100)}..."`);
    } else if (textoEstaticoBD && textoEstaticoBD.valor) {
      // Si no hay en body, usar el guardado en BD
      textoCompletado = textoEstaticoBD.valor;
      console.log(`✅ Usando texto estático de BD: "${textoCompletado.substring(0, 100)}..."`);
    } else {
      console.log(`ℹ️  Usando texto por defecto: "${textoCompletado}"`);
    }

    // Título (mantener lógica original)
    ...
    const datosPDF = {
      ...
      cuerpo: textoCompletado // 🆕 Usar el texto estático correcto
    };
    ...
    // 🆕 GUARDAR TEXTO ESTÁTICO SI SE PROPORCIONÓ
